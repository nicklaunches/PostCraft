...
export function useTemplateEditor(options: UseTemplateEditorOptions) {
    ...
    // Track if initial template has been loaded (for edit mode)
    ...
    // Track changes in name (create mode only)
    useEffect(() => {
    ...
    const handleEditorReady = useCallback(() => {
        ...
        // This prevents showing "unsaved changes" when just loading the template
        setTimeout(() => {
  ...
  const markAsChanged = useCallback(() => {
    // Only mark as changed after initial load is complete
  ...
  const initializeVariables = useCallback((templateVariables: any[]) => {
                            ...
                            toast.success(
                                mode === "create" ? "Template created!" : "Template saved!",
                                {
                                    description:
                                        mode === "create"
                                            ? `Template "${
                                                  validateTemplateName(templateName).sanitized
    ...
    }, [mode, templateName, templateId, isEditorReady, variables, router, onSaveSuccess]);
